...
%	for j=1:num_users
%		if(R(i,j)==1)
%			fh(i)=fh(i)+(((X(i)*((Theta(j))'))-Y(i,j)).^2);
%			X_grad(i)=X_grad(i)+((X(i)*((Theta(j))'))-Y(i,j))*Theta(j);
%		end
%	end
%	X_grad(i)=X_grad(i)+(lambda*(sum(sum(X(i)))));
%end
%J=(1/2)*((sum(fh))+ (lambda*((sum(sum((Theta.^2))))+(sum(sum((X.^2)))))));
...
%		end
%	end
%	Theta_grad(j)=Theta_grad(j)+(lambda*(sum(sum(Theta(j)))));
%end
